...
class State(rx.State):
    ...
    # ── Шаг 1. Быстрый обработчик загрузки (Foreground) ──────────
    ...
    # ── Шаг 2. Тяжёлый инференс в фоне (Background) ──────────
    ...
    async def run_analysis(self):
        try:
            ...
            if model_key == "orangepi":
                ...
                # ИСПРАВЛЕНО: Обязательный yield для обновления страницы
                yield
                ...
                while True:
                    ...
                    try:
                        ...
                        if failed_polls > 5:
                            async with self:
                                ...
                                self.progress_message = ""
                                self.error_message = (
                                    "Потеряна связь с устройством (Таймаут)"
                                )
                            yield
                            return
                        continue
                    ...
                    if status in ("done", "error"):
                        async with self:
                            ...
                            self.progress_message = ""
                        # ИСПРАВЛЕНО: yield для возврата текста во фронт
                        yield
                        return
                    ...
                    # ИСПРАВЛЕНО: yield для обновления таймера
                    yield
                    ...
                    try:
                        ...
                        async with self:
                            if self.cancel_requested:
                                ...
                                return
                            self.progress_message = ""
                            self.retry_message = f"Лимит запросов API. Повтор {attempt + 1}/2 через {wait} сек..."
                        yield
...
def loading_content() -> rx.Component:
    return rx.vstack(
        ...
        rx.vstack(
            ...
            rx.text(
                f"Модель: {State.model_label}", font_size="0.85em", color="#718096"
            ),
            spacing="1",
            align="center",
        ),
                    ...
                    rx.cond(
                        ...
                        rx.vstack(
                            ...
                            rx.cond(
                                ...
                                rx.button(
                                    rx.hstack(
                                        rx.icon("trash-2", size=16), rx.text("Очистить")
                                    ),
...
def llm_pending_banner() -> rx.Component:
    return rx.hstack(
        ...
        rx.text(
            rx.cond(
                State.progress_message, State.progress_message, "ИИ готовит описание…"
            ),
            font_size="0.9em",
            color="#4a5568",
            font_weight="500",
